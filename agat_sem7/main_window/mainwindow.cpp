...
//  ,
{
  ...
  connect(zoom, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          this, &MainWindow::zoomRotationChanged);
...
QMenuBar* MainWindow::getToolbar() const {
  return toolbar;
}
QFrame* MainWindow::getDataInputFrame() const {
  return dataInput;
}
  ...
  for (const QString& fileName : entries) {
    ...
    if (plugin) {
      ...
      if (iface) {
        ...
        LoadedPlugin loaded{iface, res.scene, res.dataFrame, res.menu};
